  ...
  async ({ userId, token }, thunkAPI) => {
    try {
      const response = await axios.get(
        `https://book-your-appointment.onrender.com/api/service-providers/by-user/${userId}`,
        {
          headers: {
  ...
  async ({ id, updatedData, token }, thunkAPI) => {
    try {
      const response = await axios.put(
        `https://book-your-appointment.onrender.com/api/service-providers/${id}`,
        updatedData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
...
export const deleteServiceProvider = createAsyncThunk(
  ...
  async ({ id, token }, thunkAPI) => {
    try {
      await axios.delete(`https://book-your-appointment.onrender.com/api/service-providers/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
...
export const getAllServiceProviders = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    try {
      const response = await axios.get('https://book-your-appointment.onrender.com/api/service-providers');
      return response.data;
    } catch (error) {
      return thunkAPI.rejectWithValue(error.response.data);
    }
